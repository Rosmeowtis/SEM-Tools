...
输出到 build/ 目录，包含嵌入式 Python + 后端源码 + 前端构建产物。
"""
import json
import shutil
import subprocess
...
def copy_backend():
    ...
    shutil.copytree(REPO / "backend", target,
                    ignore=shutil.ignore_patterns(".venv", "__pycache__", "*.pyc", ".python-version"))
    # 删除不需要的文件
...
def main():
    ...
    print(f"   大小: {sum(f.stat().st_size for f in BUILD.rglob('*') if f.is_file()) / 1024 / 1024:.1f} MB")
