...
from studio.models import OpBase, ReduceOpBase
...
def reduce_init(op: "ReduceOpBase") -> dict:
    """按 op.params.type 查找对应的 reduce 累加器入口。"""
    t = op.params.type  # ty:ignore[unresolved-attribute]
    entry = _REDUCE_TYPES.get(t)
    # ponytail: model_dump() 每次分配小 dict，若 profiling 显示成本可在 run_pipeline 预 dump
    return entry["init"](op.params.model_dump()) if entry else {}


def reduce_accumulate(
    op: "ReduceOpBase", state: dict, img: np.ndarray, rid: str, filename: str = ""
) -> dict:
    """按 op.params.type 将当前图像合并到累加状态中。"""
    t = op.params.type  # ty:ignore[unresolved-attribute]
    entry = _REDUCE_TYPES.get(t)
    return entry["accumulate"](state, img, rid, filename) if entry else state


def reduce_finalize(op: "ReduceOpBase", state: dict) -> dict:
    """按 op.params.type 完成聚合，返回最终结果。"""
    t = op.params.type  # ty:ignore[unresolved-attribute]
    entry = _REDUCE_TYPES.get(t)
    return entry["finalize"](state) if entry else {}


def reduce_format(op: "ReduceOpBase", state: dict) -> str:
    """按 op.params.type 将最终结果格式化为可读文本（TAB 分隔表格）。"""
    t = op.params.type  # ty:ignore[unresolved-attribute]
    entry = _REDUCE_TYPES.get(t)
